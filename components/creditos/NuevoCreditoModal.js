...
const nuevaLinea = () => ({
  ...
  total:          '',
  nota:           '',
  fecha:          fechaHoy(), // ← NUEVO: fecha actual
})
...
<div className="space-y-3">
  ...
  key={linea._id}
  className="
    rounded-xl border border-gray-400 bg-white p-3
    md:p-0 md:border-0
    grid grid-cols-[1fr_1fr_40px] gap-3
    md:grid-cols-[1fr_110px_150px_32px] md:gap-2 md:items-center
  "
>
  {/* Cliente */}
  <div className="col-span-3 md:col-span-1">
    <label className="md:hidden block text-xs text-gray-500 mb-1">
      Cliente
    </label>
  ...
  <div>
    ...
    <div className="relative">
      ...
      <input
        ...
        className="w-full pl-7 pr-2 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
      />
    </div>
  </div>
...
<input
  type="date"
  value={linea.fecha || fechaHoy()}
  onChange={e => setLinea(linea._id, { fecha: e.target.value })}
...
/>
  ...
  {/* Quitar */}
  <div className="flex items-end justify-end h-full pb-1 md:pt-0">
    <button
      type="button"
      onClick={() => quitarLinea(linea._id)}
      disabled={lineas.length === 1}
      className="
        w-9 h-9 flex items-center justify-center rounded-lg text-2xl
        text-red-400 hover:text-red-500 hover:bg-red-50 transition
